...
import { UserCog, Plus, FileUp, Trash2, Pencil } from 'lucide-react'
...
import { EditWorkcenterModal } from '@/features/workcenters/components/EditWorkcenterModal'
...
import { useDeleteWorkcenter } from '@/features/workcenters/hooks/useDeleteWorkcenter'
  ...
  return (
    <>
      ...
      {userToDelete && (
        <ConfirmModal
          title="¿Eliminar usuario?"
          message={`Vas a eliminar a "${userToDelete.first_name} ${userToDelete.last_name}". Esta acción es irreversible.`}
...
function CompaniesTable() {
  ...
  const { mutate: toggleActive, isPending: isToggling } = useToggleCompanyActive()
  const { mutate: deleteCompany, isPending: isDeleting } = useDeleteCompany()
  const [companyToDelete, setCompanyToDelete] = useState<ApiCompany | null>(null)
  ...
  return (
    <>
      ...
      {companyToDelete && (
        <ConfirmModal
          title="¿Eliminar empresa?"
          message={`Vas a eliminar "${companyToDelete.name}". Esta acción es irreversible — considera desactivarla en su lugar.`}
          confirmLabel="Eliminar"
          isPending={isDeleting}
          onConfirm={() => {
            deleteCompany(companyToDelete.uuid, {
...
function WorkcentersTable() {
  ...
  const [wcToEdit,   setWcToEdit]   = useState<ApiWorkcenter | null>(null)
  const [wcToDelete, setWcToDelete] = useState<ApiWorkcenter | null>(null)
  ...
  return (
    <>
      ...
      {wcToDelete && (
        <ConfirmModal
          title="¿Eliminar centro de trabajo?"
          message={`Vas a eliminar "${wcToDelete.name}". Esta acción es irreversible — considera desactivarlo en su lugar.`}
          confirmLabel="Eliminar"
          isPending={isDeleting}
          onConfirm={() => {
            deleteWorkcenter(wcToDelete.uuid, {
              onSuccess: () => setWcToDelete(null),
            })
          }}
          onCancel={() => setWcToDelete(null)}
        />
      )}
    </>
  )
